...
$tipo_sel =mysql_query("SELECT * FROM actas_tipo_asistentes where cod_centro='$upload_centro' and id_tipo='$id_tipo_seleccionado' ");
...
<tr>
<th width="250px" ><div id="cabecera_tabla" align='left'><?php echo "$actatexto7";?></div></th>
...
<?php
$i=1;
$tipo_asis =mysql_query("SELECT * FROM  actas_tipo_asistentes where cod_centro='$upload_centro' order by orden ");
while ($row = mysql_fetch_array($tipo_asis))
{
$id_tipo=($row ["id_tipo"]);
	if ($_SESSION["idioma_secretictac"]=='cas')
					{
				$nombre_tipo1=mysql_query("SELECT nombre_cas FROM actas_tipo_asistentes where cod_centro='$upload_centro' and id_tipo='$id_tipo' ");
				$row1 = mysql_fetch_array($nombre_tipo1);
				  $nombre_tipo_asis=$row1["nombre_cas"];
				  }
				if ($_SESSION["idioma_secretictac"]=='val')
				{
				$nombre_tipo1=mysql_query("SELECT nombre_val FROM actas_tipo_asistentes where cod_centro='$upload_centro' and id_tipo='$id_tipo' ");
				$row1 = mysql_fetch_array($nombre_tipo1);
				  $nombre_tipo_asis=$row1["nombre_val"];
				}
				
$link_editar="$ruta_absoluta/tipo_asistentes/$id_tipo";
$link_borrar="$ruta_absoluta/borrar_tipo_asistentes/$id_tipo";
...
<td >
<div id="titulo_5" align="left"><?php echo "$nombre_tipo_asis";?>
</div>
</td>
...
<a href='<?php echo "$link_borrar";?>' onclick="return confirmar('<?php echo "$actatexto16  $nombre_tipo_asis?";?>')" target="_self">
...
<?php
if (!isset($_REQUEST['id_tipo']))
$texto_cabecera_formulario=$actatexto13;
else
...
<form  name="Form1" class="formulario_borde2" method="post" action="<?php echo "$ruta_absoluta";?>/upload_tipo_asistente">
...
<input type='text' maxlength='2' autocomplete="off" style='width:40px;' name='orden'  value='<?php echo "$orden";?>'/>
</div>
