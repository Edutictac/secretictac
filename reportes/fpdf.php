...
//COMIENZA LA CLASE PARA DIBUJAR INAGENES
...
function drawTextBox($strText, $w_1, $h_1, $align='L', $valign='T', $border=true)
{
    $xi=$this->GetX();
    $yi=$this->GetY();

    $hrow=$this->FontSize;
    $textrows=$this->drawRows($w_1,$hrow,$strText,0,$align,0,0,0);
    $maxrows=floor($h_1/$this->FontSize);
    $rows=min($textrows,$maxrows);

    $dy=0;
    if (strtoupper($valign)=='M')
        $dy=($h_1-$rows*$this->FontSize)/2;
    if (strtoupper($valign)=='B')
        $dy=$h_1-$rows*$this->FontSize;

    $this->SetY($yi+$dy);
    $this->SetX($xi);

    $this->drawRows($w_1,$hrow,$strText,0,$align,false,$rows,1);

    if ($border)
        $this->Rect($xi,$yi,$w_1,$h_1);
}

function drawRows($w_1, $h_1, $txt, $border=0, $align='J', $fill=false, $maxline=0, $prn=0)
{
    $cw=&$this->CurrentFont['cw'];
    if($w_1==0)
        $w_1=$this->w-$this->rMargin-$this->x;
    $wmax=($w_1-2*$this->cMargin)*1000/$this->FontSize;
    $s=str_replace("\r",'',$txt);
    $nb=strlen($s);
    if($nb>0 && $s[$nb-1]=="\n")
        $nb--;
    ...
    while($i<$nb)
    {
        ...
        if($c=="\n")
        {
            ...
            if ($prn==1) {
                $this->Cell($w_1,$h_1,substr($s,$j,$i-$j),$b,2,$align,$fill);
            }
            $i++;
            $sep=-1;
            $j=$i;
        ...
        if($l>$wmax)
        {
            ...
            else
            {
                ...
                if ($prn==1){
                    $this->Cell($w_1,$h_1,substr($s,$j,$sep-$j),$b,2,$align,$fill);
                }
                $i=$sep+1;
            }
            $sep=-1;
...
function MemImage($data, $x=null, $y=null, $w=0, $h=0, $link='')
    {
        ...
        */
        ...
        //esto proximo es nuevo, borrar si no vale
        $h=15;
        $w=$anchura/$altura*15;
        $_SESSION['alto_imagen']=$h;
        $_SESSION['ancho_imagen']=$w;
...
function MemImage_pie($data, $x=null, $y=null, $w=0, $h=0, $link='')
    ...
    function GDImage_pie($im, $x=null, $y=null, $w=0, $h=0, $link='')
    {
        //Display the GD image associated to $im
        ob_start();
        imagepng($im);
        $data = ob_get_clean();
        $this->MemImage_pie($data, $x, $y, $w, $h, $link);
    }
...
function FPDF($orientation='P', $unit='mm', $format='A4')
{
	...
	$this->ws=0;
	//declaracion de var para imagenes
	...
	$this->PageFormats=array('a3'=>array(841.89,1190.55), 'a4'=>array(595.28,841.89), 'a5'=>array(420.94,595.28),
		'letter'=>array(612,792), 'legal'=>array(612,1008),'carnet'=>array(595.28,802.20));
	if(is_string($format))
...
function _putresources1()
{
    parent::_putresources1();
    $this->_putbookmarks();
}

function _putcatalog1()
{
    parent::_putcatalog1();
